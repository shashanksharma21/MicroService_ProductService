...
public class ProductServiceImpl<T> implements ProductService<T> {
	...
	@Override
	public T createProduct(Product productDto) {
	...
	@Override
	public List<ProductResponseDto> getProductByUserId(String userId) {
		List<Product> productByUserId = this.productRepository.findByUserId(userId);
		List<ProductResponseDto> allProductDtos = productByUserId.stream().map( product -> this.modelMapper.map(product, ProductResponseDto.class)).collect(Collectors.toList());
